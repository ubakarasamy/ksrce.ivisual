...
<div class="col-md-6">
  ...
  <input type="text" class="form-control" style="width:250px;" name="name" id="name" v-model="StaffNameSearch" >
</div>
...
export default {
    ...
    methods:{
      getoverallCount(){
   fetch('/api/academic/staffoverall')
            .then(res => res.json())
            .then(res => {
                this.overCount = res.data;
...
<style scoped>
.department-filter input, .department-filter label, .department-filter select
{
  display: inline;
}
.department-filter
