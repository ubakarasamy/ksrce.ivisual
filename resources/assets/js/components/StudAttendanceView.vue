...
<p class="float-right text-dark">
  If Statuses are NULL the respected staff has not updated Attendance
</p>
...
<!-- get data -->
<button @click="ClickAllPresent()" class="btn btn-outline-primary">All Present</button>
<button @click="ClickAllAbsent()" class="btn btn-outline-primary">All Absent</button>
 <table class="table">
                    <thead>
                        <th>#REG NO</th>
                        <th>Name</th>
                    ...
                    <tbody>
                        <tr v-for="student in filteredStudents" v-bind:key="student.id" v-if="hideTable === false">
                            ...
                            <td>
                                <ul class="hours_list">
                                    ...
                                    <li>
                                      {{getStatusAll(student.id)}}
                                    </li>
                                </ul>
                            </td>
                        </tr>
                        <tr v-for="student in filteredStudents" v-bind:key="student.id" v-if="AllPresent === true">
                            <td v-if="getStatusAll(student.id) == 7">{{ student.register_no }}</td>
                            <td v-if="getStatusAll(student.id) == 7">{{ student.name }}</td>
                            <td v-if="getStatusAll(student.id) == 7">
                                <ul class="hours_list">
                                    <li>
                                        1 <br>
                                        <span v-if="getStatusH1(student.id)" style="color:#89729E;">{{getStatusH1(student.id)}}</span>
                        ...
                        <tr v-for="student in filteredStudents" v-bind:key="student.id" v-if="AllAbsent === true">
                            <td v-if="getStatusAll(student.id) == 0 | getStatusAll(student.id) == null">{{ student.register_no }}</td>
                            <td v-if="getStatusAll(student.id) == 0 | getStatusAll(student.id) == null">{{ student.name }}</td>
                            <td v-if="getStatusAll(student.id) == 0 | getStatusAll(student.id) == null">
                                <ul class="hours_list">
                                    <li>
                                        1 <br>
                                        <span v-if="getStatusH1(student.id)" style="color:#89729E;">{{getStatusH1(student.id)}}</span>
