...
<div class="form-group">
  <input v-model="searchFrm" placeholder="Search by Name or REG No" type="text" style="width:250px;" class="form-control mt-3">
</div>
<div class="row">
<div class="filter-options col-md-12" style="background: #f2f2f2;
    padding: 10px 10px;">
    <div class="inner-div ml-3">
<label for="student_degree_filter">Degree</label>
<select :disabled="userrole > 2" class="form-control" id="student_degree_filter" style="width:120px;display:inline-block;" v-model="student_degree_filter_selected">
...
<select :disabled="userrole > 2" class="form-control" id="student_department_filter" style="width:120px;display:inline-block;" v-model="student_department_filter_selected">
...
<select :disabled="userrole > 2" class="form-control" id="student_filter_department" v-model="student.department" style="width:120px;">
...
student_section_create_options: [
 ...
 semesterOptions:[
    ...
    getDegree(department){
      ...
      var aData;
      var child;
      aData = this.student_department_filter_options;
 ...
 if(this.userrole > 2){
        this.student_department_filter_selected = this.authenticateduser.department;
        this.validateDepart = 1;
        this.student_degree_filter_selected = this.getDegree(this.authenticateduser.department);
      }
        ...
        if(filterDepartment === 'all' && filterYear === 'all' && filterSection === 'all' && filterDegree === 'all' && search === ''){
          return vm.students;
        }else{
          return vm.students.filter(function(student){
            return (filterDepartment === 'all' || student.department === filterDepartment) 
            && (filterYear === 'all' || student.year === filterYear) 
            && (filterSection === 'all' || student.section === filterSection) 
            && (filterDegree === 'all' || student.degree === filterDegree)
            && (student.name.toLowerCase().includes(search.toLowerCase()) || student.register_no.toLowerCase().includes(search.toLowerCase()));
